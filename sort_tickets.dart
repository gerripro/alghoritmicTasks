...
void main() {
  ...
  String beginDestination =
      citiesFrom.firstWhere((element) => !citiesTo.contains(element));

  List sortedTickets = [];

  sortedTickets.add(
      tickets.firstWhere((element) => element['from'] == beginDestination));

  while (sortedTickets.length != tickets.length) {
    sortedTickets.add(tickets
        .firstWhere((element) => element['from'] == sortedTickets.last['to']));
  }
